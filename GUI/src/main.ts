import { login, upload, UserToken } from '@micrio/tiler-base';
...
const createWindow = (): void => {
	...
	// Load renderer from Vite dev server in development, or the packaged renderer HTML in production.
	...
	mainWindow.webContents
		.executeJavaScript('localStorage.getItem("account")', true)
		.then(
			(acct: string) => {
				if (acct)
					try {
...
app.whenReady().then(() => {
	...
	ipcMain.on('login', () =>
		login((url) => updateState((state.loginUrl = url))).then(
			(account: UserToken) =>
	...
	ipcMain.on('logout', () => {
		...
		mainWindow.webContents
			.executeJavaScript(`localStorage.removeItem('account')`, true)
	...
	ipcMain.on('start', () => {
		...
		upload(
			...
			(e) => {
				console.error(e);
				updateState(
					(state.error = 'Error: ' + (e?.message ?? 'Unknown error')),
				);
			},
		);
	});
	...
	ipcMain.on('destination', (e, slug: string) => {
		if (slug) {
			...
			saveAccount(account).then(() =>
				updateState(
					(state.destination = `https://dash.micr.io/${slug}`),
				),
			);
		} else updateState(delete state.destination);
	});
	ipcMain.on('isOmni', (e, checked: boolean) =>
		updateState((state.type = checked ? 'omni' : '2d')),
	);
